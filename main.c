...
#define BT_EMU_CTRL_PIN 3   // GPIO3 -> control line to second RP2040 (BT emulator).
                            // HIGH = BT visible on USB, LOW = BT disconnected.
...
// BT emulator control state machine
//   bt_hide_until_ms = 0   -> BT visible (GPIO HIGH on BT_EMU_CTRL_PIN)
//   bt_hide_until_ms != 0  -> BT hidden until this absolute time, then auto-restore
static volatile uint32_t bt_hide_until_ms = 0;
// Track UP/DOWN sequence to detect "WiFi enable" pattern.
// First UP after boot is normal init. DOWN-then-UP again means user toggled WiFi.
static volatile uint8_t up_count = 0;
static volatile bool last_was_down = false;
...
//--------------------------------------------------------------------
// BT emulator control (second RP2040 on hub port 2)
//
// Wiring: GPIO BT_EMU_CTRL_PIN -> input pin on second RP2040
//         Common GND between both boards.
//
// Logic:
//   HIGH = BT visible on USB bus (default)
//   LOW  = BT hidden (second RP2040 calls tud_disconnect())
//
// We hide BT for HIDE_DURATION_MS when plotter enables WiFi (detected by
// DOWN -> UP sequence). Main loop polls bt_hide_until_ms and restores HIGH
// when timer elapses. RP2040 keeps talking to plotter the whole time.
//--------------------------------------------------------------------
#define BT_HIDE_DURATION_MS  6000u

static void bt_emu_init(void) {
    gpio_init(BT_EMU_CTRL_PIN);
    gpio_set_dir(BT_EMU_CTRL_PIN, GPIO_OUT);
    gpio_put(BT_EMU_CTRL_PIN, 1);  // BT visible by default
    bt_hide_until_ms = 0;
    printf("BT emulator control: GPIO%d HIGH (BT visible)\r\n", BT_EMU_CTRL_PIN);
}

static void bt_emu_hide_for_ms(uint32_t duration_ms) {
    uint32_t now_ms = to_ms_since_boot(get_absolute_time());
    bt_hide_until_ms = now_ms + duration_ms;
    gpio_put(BT_EMU_CTRL_PIN, 0);
    printf("BT emulator: HIDE for %lu ms (until %lu ms)\r\n",
           (unsigned long)duration_ms, (unsigned long)bt_hide_until_ms);
}

static void bt_emu_restore(void) {
    bt_hide_until_ms = 0;
    gpio_put(BT_EMU_CTRL_PIN, 1);
    printf("BT emulator: RESTORE (BT visible again)\r\n");
}

// Called from main loop. Restores BT after hide timer elapses.
static void bt_emu_tick(void) {
    if (bt_hide_until_ms == 0) return;  // not hiding
    uint32_t now_ms = to_ms_since_boot(get_absolute_time());
    if ((int32_t)(now_ms - bt_hide_until_ms) >= 0) {
        bt_emu_restore();
    }
}
            ...
            case BRCMF_C_UP:
                {
                    ...
                    // Detect "WiFi enable" pattern:
                    // First UP after boot is normal init -- don't hide BT.
                    // Subsequent UP after a DOWN means user toggled WiFi ON
                    // in plotter settings. THAT is when plotter ruga"etsa
                    // on the BT device presence. Hide BT for 6 seconds.
                    bool is_wifi_enable = (up_count > 1) && last_was_down;
                    last_was_down = false;

                    printf("\r\n===== BRCMF_C_UP =====\r\n");
                    printf("  up_time_ms    = %lu\r\n", up_time);
                    printf("  up_count      = %u\r\n", up_count);
                    printf("  is_wifi_enable= %d\r\n", is_wifi_enable);
                    printf("  dongle_up     = 1\r\n");
                    printf("  boot_mode     = %d\r\n", boot_mode);
                    ...
                    if (is_wifi_enable) {
                        // Hide BT during plotter's WiFi-enable phase.
                        // Plotter expects BT to "go away" exactly here -- this is what
                        // physical "remove flash drive" did manually before.
                        bt_emu_hide_for_ms(BT_HIDE_DURATION_MS);
                    }
...
static void switch_to_runtime_reconnect(void) {
    ...
    // Reset BT/UP tracking on reconnect (treat next UP as first init, not WiFi enable)
    up_count = 0;
    last_was_down = false;
    if (bt_hide_until_ms != 0) {
        bt_emu_restore();
    }
    status_led_runtime();
    for (int i = 0; i < 10; i++) {
...
int main(void) {
    ...
    // Initialize BT emulator control GPIO (BT visible by default)
    bt_emu_init();
    ...
    while (1) {
        tud_task();
        drain_vendor_out();
        bt_emu_tick();          // restore BT after hide timer elapses
