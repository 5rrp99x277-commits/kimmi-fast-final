            ...
            case BRCMF_C_UP:
                {
                    ...
                    // Trigger BT disconnect: GPIO3 HIGH for 3 seconds.
                    // The second RP2040 (BT dummy) sees HIGH → disconnects
                    // from USB → Canon thinks BT radio powered down.
                    gpio_put(BT_TRIGGER_PIN, 1);  // HIGH → BT disconnect
                    printf("BT trigger: GPIO%d HIGH (3s)\r\n", BT_TRIGGER_PIN);

                    // Poll tud_task() for 3000ms while BT is "off"
                    uint32_t bt_off_start = to_ms_since_boot(get_absolute_time());
                    while ((to_ms_since_boot(get_absolute_time()) - bt_off_start) < 3000) {
                        tud_task();
                        sleep_ms(10);
                    }
