...
static volatile uint32_t pending_if_event_ms = 0;  // when to send WLC_E_IF (0=never)
static volatile uint32_t pending_scan_complete_ms = 0;  // when to send WLC_E_SCAN_COMPLETE (0=never)
...
#define CYW4373_BOARDTYPE  0x084Au
...
static void open_ep1_in(void) {
    ...
    memset(&ep1_desc, 0, sizeof(ep1_desc));
    ep1_desc.bLength          = sizeof(tusb_desc_endpoint_t);
    ep1_desc.bDescriptorType  = TUSB_DESC_ENDPOINT;
    ep1_desc.bEndpointAddress = 0x81;
    // pico-sdk TinyUSB uses bitfield struct for bmAttributes -- assign .xfer
    ep1_desc.bmAttributes.xfer = TUSB_XFER_INTERRUPT;  // 0x03
...
static void send_interrupt_event(void) {
    ...
    if (!usbd_edpt_claim(0, 0x81)) {
        printf("EP81 send: claim FAIL\r\n");
        return;
    }
    ...
    if (!xfer_ok) {
        usbd_edpt_release(0, 0x81);
        printf("EP81 send: released after failed xfer\r\n");
    }
    // If xfer_ok: TinyUSB releases endpoint automatically when transfer completes
}

//--------------------------------------------------------------------
// Send Broadcom event packet on EP82 (bulk IN) -- 77 bytes total
//
// Format per brcmfmac (Linux kernel):
//   [0..13]   Ethernet header (14B):
//                 dst=FF*6, src=EMU_MAC, type=0x886C BE
//   [14..23]  bcmeth_hdr (10B):
//                 subtype=0x8001 BE, length=0x003B BE,
//                 version=0, oui=00:10:18, usr_subtype=0x0001 BE
//   [24..71]  wl_event_msg (48B, ALL FIELDS BIG-ENDIAN):
//                 version, flags, event_type, status, reason, auth_type,
//                 datalen, addr(6), ifname(16), ifidx(1), bsscfgidx(1)
//   [72..76]  payload (5B): wl_event_data_if for WLC_E_IF
//                 ifidx, action, flags, bssidx, role
//
// Critical: drivers check `pktlen < sizeof(bcm_event_t)` and DROP if smaller.
// 64 bytes was being silently dropped by Canon firmware.
//--------------------------------------------------------------------
static void send_bulk_event(uint32_t event_type, uint32_t status, uint32_t reason, uint32_t flags) {
    uint8_t pkt[80];
    memset(pkt, 0, sizeof(pkt));

    // ---- Ethernet header (offset 0..13) ----
    // dst = broadcast
    ...
    // ---- bcmeth_hdr (offset 14..23) ----
    pkt[14]=0x80; pkt[15]=0x01;        // subtype = BCMILCP_SUBTYPE_VENDOR_LONG (0x8001 BE)
    pkt[16]=0x00; pkt[17]=0x3B;        // length = 59 BE (everything after this field = 1+3+2+48+5)
    pkt[18]=0x00;                      // version
    pkt[19]=0x00; pkt[20]=0x10; pkt[21]=0x18;  // OUI = Broadcom
    pkt[22]=0x00; pkt[23]=0x01;        // usr_subtype = 0x0001 BE (BCMILCP_BCM_SUBTYPE_EVENT)

    // ---- wl_event_msg (offset 24..71, ALL BIG-ENDIAN) ----
    // version (u16) = 2
    ...
    // auth_type (u32) = 0 (offset 40..43 already 0)
    // datalen (u32) BE = 5 (size of payload below)
    pkt[44]=0x00; pkt[45]=0x00; pkt[46]=0x00; pkt[47]=0x05;
    // addr (6B) at offset 48..53 = our MAC
    memcpy(pkt + 48, EMU_MAC, 6);
    // ifname (16B) at offset 54..69 = "wl0"
    memcpy(pkt + 54, "wl0", 3);
    // ifidx (1B) at offset 70 = 0
    // bsscfgidx (1B) at offset 71 = 0

    // ---- wl_event_data_if payload (offset 72..76, 5 bytes) ----
    pkt[72]=0x00;       // ifidx
    pkt[73]=0x01;       // action = BRCMF_E_IF_ADD
    pkt[74]=0x00;       // flags
    pkt[75]=0x00;       // bssidx
    pkt[76]=0x00;       // role = STA

    // ---- Send 77 bytes via EP82 bulk ----
    // tud_vendor_write may need multiple calls because MPS=64 and total=77
    uint16_t total = 77;
    uint16_t sent = 0;
    int retries = 10;
    while (sent < total && retries-- > 0) {
        uint32_t n = tud_vendor_write(pkt + sent, total - sent);
        sent += n;
        if (n == 0) {
            tud_task();  // give TinyUSB chance to drain FIFO
        }
    }
    tud_vendor_flush();
    printf("EP82 event %lu sent: %u/%u bytes\r\n",
           (unsigned long)event_type, sent, total);
}

//--------------------------------------------------------------------
// Send WLC_E_ESCAN_RESULT event with empty result (no networks).
//
// Real firmware sends escan result with embedded wl_escan_result_t payload:
//   uint32_t buflen
//   uint32_t version (= WL_BSS_INFO_VERSION = 109 typical)
//   uint16_t sync_id
//   uint16_t bss_count = 0   <-- key: zero networks
//   wl_bss_info_t bss_info[]; (empty when bss_count=0)
//
// Total payload = 12 bytes (just header, no bss_info entries).
// Sent through send_bulk_event with proper bcm_event_t wrapper.
//
// We piggyback on send_bulk_event() but need to extend payload to include
// the escan_result struct. For simplicity: send as event with datalen=12
// and append the escan_result header after the standard 5-byte placeholder.
//--------------------------------------------------------------------
static void send_empty_escan_result(uint16_t sync_id) {
    // Build same event frame as send_bulk_event but with custom payload (12 bytes).
    // Total: 14 (eth) + 10 (bcmeth) + 48 (event_msg) + 12 (escan payload) = 84 bytes
    uint8_t pkt[96];
    memset(pkt, 0, sizeof(pkt));
    ...
    pkt[16]=0x00; pkt[17]=0x42;            // length = 66 BE (1+3+2+48+12)
    pkt[18]=0x00;
    pkt[19]=0x00; pkt[20]=0x10; pkt[21]=0x18;
    pkt[22]=0x00; pkt[23]=0x01;

    // ---- wl_event_msg (48B BE) ----
    pkt[24]=0x00; pkt[25]=0x02;            // version=2
    pkt[26]=0x00; pkt[27]=0x00;            // flags=0
    pkt[28]=0x00; pkt[29]=0x00; pkt[30]=0x00; pkt[31]=69;   // event_type = WLC_E_ESCAN_RESULT
    pkt[32]=0x00; pkt[33]=0x00; pkt[34]=0x00; pkt[35]=0x00; // status=SUCCESS
    pkt[36]=0x00; pkt[37]=0x00; pkt[38]=0x00; pkt[39]=0x00; // reason=0
    pkt[40]=0x00; pkt[41]=0x00; pkt[42]=0x00; pkt[43]=0x00; // auth_type=0
    pkt[44]=0x00; pkt[45]=0x00; pkt[46]=0x00; pkt[47]=12;   // datalen=12
    memcpy(pkt + 48, EMU_MAC, 6);
    memcpy(pkt + 54, "wl0", 3);

    // ---- wl_escan_result payload (offset 72, 12 bytes) ----
    // buflen (u32 LE = host byte order in escan payload)
    pkt[72]=12; pkt[73]=0; pkt[74]=0; pkt[75]=0;
    // version (u32 LE)
    pkt[76]=109; pkt[77]=0; pkt[78]=0; pkt[79]=0;
    // sync_id (u16 LE)
    pkt[80]=(uint8_t)(sync_id & 0xFF);
    pkt[81]=(uint8_t)((sync_id >> 8) & 0xFF);
    // bss_count = 0 (u16 LE)
    pkt[82]=0; pkt[83]=0;
    ...
    tud_vendor_flush();
    printf("EP82 escan_result (empty) sent: %u/%u bytes sync=%u\r\n",
           sent, total, sync_id);
}
...
static uint16_t prepare_bcdc_response(uint8_t *buf, uint16_t max_len) {
    ...
    if (effective_cmd < 256) {
        switch (effective_cmd) {
            case BRCMF_C_UP:
                printf("RUNTIME BRCMF_C_UP -> OK, scheduling WLC_E_IF in 150ms\r\n");
                dongle_up = true;
                send_interrupt_event();          // short ping on EP81
                // Real firmware doesn't send the IF_ADD event instantly --
                // schedule it for ~150ms later, polled in main loop.
            ...
            case BRCMF_C_DOWN:
                ...
                break;
            case BRCMF_C_SCAN:
                // Old-style scan IOCTL (legacy). Some drivers use this instead of
                // SET_VAR escan. Schedule SCAN_COMPLETE to come back in 1.5 sec.
                printf("RUNTIME BRCMF_C_SCAN (ioctl=50) -> OK, scheduling SCAN_COMPLETE\r\n");
                pending_scan_complete_ms = to_ms_since_boot(get_absolute_time()) + 1500;
                break;
            case BRCMF_C_SCAN_RESULTS:
                // Old-style: return empty result list (4-byte buflen=0 + nothing)
                printf("RUNTIME BRCMF_C_SCAN_RESULTS (ioctl=51) -> empty\r\n");
                payload_len = max_payload >= 4 ? 4 : max_payload;
                memset(payload, 0, payload_len);  // count=0
                break;
            case BRCMF_C_DISASSOC:
                // Disassociate: nothing to do, we're never associated
                printf("RUNTIME BRCMF_C_DISASSOC -> OK\r\n");
                break;
            ...
            case BRCMF_C_SET_PM_COMPAT:
                printf("RUNTIME BRCMF_C_SET_PM/86 -> OK\r\n");
                payload_len = 0;
                break;
    ...
    if (effective_cmd == BRCMF_C_GET_VAR) {
        ...
        // ===== WiFi association state (required for "ready but not connected") =====
            ...
            // Not associated -> RSSI = 0 (or -100 dBm = 0xFFFFFF9C signed)
            payload_len = copy_u32_payload(payload, max_payload, 0);

        } else if (strncmp(iovar, "assoc_info", 20) == 0) {
            // assoc_info: empty when not associated
            payload_len = max_payload >= 8 ? 8 : max_payload;
            memset(payload, 0, payload_len);

        } else if (strncmp(iovar, "chanspec", 20) == 0) {
            // chanspec: 0 when not associated
            payload_len = copy_u32_payload(payload, max_payload, 0);
            ...
            payload_len = copy_u32_payload(payload, max_payload, 40);

        } else if (strncmp(iovar, "scan_unassoc_time", 20) == 0) {
            payload_len = copy_u32_payload(payload, max_payload, 80);

        } else if (strncmp(iovar, "scan_passive_time", 20) == 0) {
            payload_len = copy_u32_payload(payload, max_payload, 110);

        } else if (strncmp(iovar, "pm", 20) == 0) {
            // power management: 0 = off (CAM mode)
            payload_len = copy_u32_payload(payload, max_payload, 0);
            ...
            // Return empty result list (no networks visible).
            // Structure: u32 buflen=4 + u32 version=109 + u16 sync_id=0 + u16 bss_count=0 = 12 bytes
            uint16_t total_len = 12;
            payload_len = total_len < max_payload ? total_len : max_payload;
            memset(payload, 0, payload_len);
            if (payload_len >= 4) {
                payload[0] = 12; payload[1] = 0; payload[2] = 0; payload[3] = 0;  // buflen
            }
            if (payload_len >= 8) {
                payload[4] = 109; payload[5] = 0; payload[6] = 0; payload[7] = 0; // version
            }
            // sync_id at [8..9] = 0
            // bss_count at [10..11] = 0 (no networks)
            printf("RUNTIME answer %s with empty list (0 networks)\r\n", iovar);
        ...
        payload_len = 0;
        // After scan/escan request, schedule deferred SCAN_COMPLETE event.
        // Real firmware takes ~1-3 sec to scan all channels and reports SUCCESS
        // with 0 networks (not NO_NETWORKS error -- that means scan failed).
        if (last_iovar_valid && (strncmp(set_name, "escan", 20) == 0 ||
                                  strncmp(set_name, "scan", 20) == 0)) {
            printf("RUNTIME scan triggered, scheduling SCAN_COMPLETE in 1500ms\r\n");
            pending_scan_complete_ms = to_ms_since_boot(get_absolute_time()) + 1500;
        }
...
bool tud_vendor_control_xfer_cb(uint8_t rhport, uint8_t stage, tusb_control_request_t const * request) {
    // Diagnostic: log every SETUP control transfer in runtime mode after BRCMF_C_UP.
    // Helps identify what plotter sends in the 5 sec after USB suspend.
    if (!boot_mode && stage == CONTROL_STAGE_SETUP && dongle_up) {
        uint32_t now_ms = to_ms_since_boot(get_absolute_time());
        printf("CTRL setup at %lu ms: bmRT=0x%02X bReq=0x%02X wVal=0x%04X wIdx=0x%04X wLen=%u\r\n",
...
static void drain_vendor_out(void) {
    ...
    while (tud_vendor_available()) {
        ...
        if (boot_mode) {
            ...
            // Diagnostic: log ANY bulk OUT activity in runtime mode with timestamp.
            // After USB suspend, this catches what plotter sends in the 5 sec
            // before "incompatible device" error.
            uint32_t now_ms = to_ms_since_boot(get_absolute_time());
            printf("RUNTIME bulk OUT %lu bytes at %lu ms (first 16:",
                   (unsigned long)count, (unsigned long)now_ms);
            uint32_t dump_n = count < 16 ? count : 16;
...
void tud_suspend_cb(bool remote_wakeup_en) {
    uint32_t now_ms = to_ms_since_boot(get_absolute_time());
    printf("USB suspend (remote_wakeup_en=%d) at %lu ms\r\n", remote_wakeup_en, (unsigned long)now_ms);
    status_led_off();
    // Stop heartbeat during suspend -- USB spec requires no traffic on suspended bus.
    // Plotter measures bus quietness; if we keep sending, it thinks module is broken.
    if (!boot_mode) {
        dongle_up = false;
        printf("Suspend: heartbeat stopped\r\n");
    }
}
void tud_resume_cb(void) {
    uint32_t now_ms = to_ms_since_boot(get_absolute_time());
    printf("USB resume at %lu ms\r\n", (unsigned long)now_ms);
    if (!boot_mode) {
        status_led_runtime();
        // Resume heartbeat -- module is awake again
        dongle_up = true;
        printf("Resume: heartbeat restored\r\n");
        send_interrupt_event();
    } else {
        status_led_boot();
    }
...
// Diagnostic: USB bus reset detection -- if plotter does usb_reset_device()
// after suspend, this will fire. Real CYW4373 returns to boot mode on reset.
void tud_reset_cb(void) {
    uint32_t now_ms = to_ms_since_boot(get_absolute_time());
    printf("USB BUS RESET at %lu ms (boot_mode=%d, dongle_up=%d)\r\n",
           (unsigned long)now_ms, boot_mode, dongle_up);
}
...
static void switch_to_runtime_reconnect(void) {
    ...
    // Reset state from previous session to avoid stale events
    dongle_up = false;
...
int main(void) {
    ...
    printf("\r\n=== CYW4373 Emulator v4.19 fixed + runtime BCDC debug patch ===\r\n");
    ...
    while (1) {
        ...
        // Deferred WLC_E_IF event: triggered by BRCMF_C_UP, fires after 150ms.
        // Real firmware doesn't send IF_ADD instantly -- this mimics that timing.
        if (!boot_mode && pending_if_event_ms != 0) {
            uint32_t now_ms = to_ms_since_boot(get_absolute_time());
            if ((int32_t)(now_ms - pending_if_event_ms) >= 0) {
                pending_if_event_ms = 0;
                printf("Deferred BRCMF_E_IF -> send_bulk_event\r\n");
                send_bulk_event(54, 0, 0, 1);   // WLC_E_IF=54, status=SUCCESS, flags=UP
            }
        }

        // Deferred SCAN_COMPLETE event: triggered by escan/scan request, fires after 1.5 sec.
        // Real firmware scans all channels (~1-3 sec), then sends:
        //   1. WLC_E_ESCAN_RESULT (event=69) with empty list  -- status=SUCCESS, bss_count=0
        //   2. WLC_E_SCAN_COMPLETE (event=52)                 -- status=SUCCESS
        // status=SUCCESS (0), NOT NO_NETWORKS (3). status=3 means scan FAILED, not "no networks visible".
        if (!boot_mode && pending_scan_complete_ms != 0) {
            uint32_t now_ms = to_ms_since_boot(get_absolute_time());
            if ((int32_t)(now_ms - pending_scan_complete_ms) >= 0) {
                pending_scan_complete_ms = 0;
                printf("Deferred SCAN_COMPLETE -> sending empty scan result\r\n");
                send_empty_escan_result(0);     // WLC_E_ESCAN_RESULT with bss_count=0
                send_bulk_event(52, 0, 0, 0);   // WLC_E_SCAN_COMPLETE=52, status=SUCCESS
            }
        }
