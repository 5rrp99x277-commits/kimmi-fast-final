    ...
    if (effective_cmd < 256) {
        switch (effective_cmd) {
            ...
            case BRCMF_C_DOWN:
                ...
                // Long pulse 100ms = BT connect (show BT — WiFi off, BT should be visible)
                gpio_put(BT_TRIGGER_PIN, 1);
                busy_wait_ms(100);
                gpio_put(BT_TRIGGER_PIN, 0);
...
void tud_resume_cb(void) {
    ...
    // Short pulse: BT hide (runtime reconnect with active WiFi)
    gpio_put(BT_TRIGGER_PIN, 1);
    busy_wait_ms(1);
    gpio_put(BT_TRIGGER_PIN, 0);
    printf("[RESUME] BT hide pulse\r\n");
