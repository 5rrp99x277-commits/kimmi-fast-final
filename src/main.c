...
static uint32_t bt_trigger_end_ms = 0;  // 0=inactive, >0=time to lower GPIO3
    ...
    if (effective_cmd < 256) {
        switch (effective_cmd) {
            case BRCMF_C_UP:
                {
                    ...
                    // Trigger BT disconnect: GPIO3 HIGH for 6 seconds.
                    // Defer to main loop — do NOT block here or the host's
                    // control transfers during this window will time out.
                    gpio_put(BT_TRIGGER_PIN, 1);  // HIGH → BT disconnect
                    bt_trigger_end_ms = up_time + 6000;
                    printf("BT trigger: GPIO%d HIGH until %lu\r\n", BT_TRIGGER_PIN, bt_trigger_end_ms);
            ...
            case BRCMF_C_DOWN:
                ...
                dongle_up = false;
                pending_if_event_ms = 0;  // cancel deferred IF — host is going down
                break;
            case BRCMF_C_SCAN:
...
int main(void) {
    ...
    while (1) {
        ...
        // BT trigger: lower GPIO3 after 6 seconds (non-blocking)
        if (bt_trigger_end_ms != 0) {
            uint32_t now_ms = to_ms_since_boot(get_absolute_time());
            if ((int32_t)(now_ms - bt_trigger_end_ms) >= 0) {
                gpio_put(BT_TRIGGER_PIN, 0);  // LOW → BT can reconnect
                printf("BT trigger: GPIO%d LOW\r\n", BT_TRIGGER_PIN);
                bt_trigger_end_ms = 0;
            }
        }
