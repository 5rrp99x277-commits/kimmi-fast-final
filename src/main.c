...
static void open_ep1_in(void) {
    ...
    // In pico-sdk TinyUSB bmAttributes is an anonymous struct with bit fields
    ep1_desc.xfer             = TUSB_XFER_INTERRUPT;  // 3
    ep1_desc.sync             = 0;
    ep1_desc.usage            = 0;
    ep1_desc.wMaxPacketSize   = 16;             // 16 bytes
    ep1_desc.bInterval        = 9;              // 9 ms polling interval
    bool ok = tud_edpt_open(&ep1_desc);         // pico-sdk: no rhport arg
    if (ok) {
        ep1_in_opened = true;
        printf("EP81 open: OK (via tud_edpt_open)\r\n");
        return;
    }
    // If tud_edpt_open failed, endpoint might already be opened by TinyUSB
    // from the configuration descriptor at SET_CONFIGURATION time.
    // Test by checking if endpoint is busy (returns true if active).
    if (tud_edpt_busy(0x81)) {
        ep1_in_opened = true;
...
static void send_interrupt_event(void) {
    ...
    bool xfer_ok = tud_edpt_xfer(0x81, evt, sizeof(evt));
