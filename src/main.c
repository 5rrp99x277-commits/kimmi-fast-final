...
//--------------------------------------------------------------------
// Send short interrupt notification on EP81 (16 bytes max)
// This is just a "ping" that activity exists; full event goes on EP82 bulk IN.
//--------------------------------------------------------------------
static void send_interrupt_event(void) {
    uint8_t evt[16];
    memset(evt, 0, sizeof(evt));
    // Short notification: event_type = WLC_E_IF (54) + flags = IF_UP
    evt[0] = 0x36;  // event type low byte (WLC_E_IF)
    evt[4] = 0x01;  // flags: interface up
...
static void send_bulk_event(uint32_t event_type, uint32_t status, uint32_t reason, uint32_t flags) {
    ...
    // Ethernet header: dst=broadcast, src=zeros, type=0x886C (Broadcom)
    pkt[0] = 0xFF; pkt[1] = 0xFF; pkt[2] = 0xFF; pkt[3] = 0xFF; pkt[4] = 0xFF; pkt[5] = 0xFF;
    // src = zeros (6 bytes) at pkt[6..11]
    pkt[12] = 0x88; pkt[13] = 0x6C;  // Broadcom ethertype
    // BCM ethertype header: subtype=0x01 (event), version=1
    pkt[14] = 0x01;
    pkt[15] = 0x01;
    // Event message starts at pkt[16]
    // event_type (u16)
    pkt[16] = (uint8_t)(event_type & 0xFF);
    pkt[17] = (uint8_t)((event_type >> 8) & 0xFF);
    // status (u32)
    pkt[20] = (uint8_t)(status & 0xFF);
    pkt[21] = (uint8_t)((status >> 8) & 0xFF);
    // reason (u32)
    pkt[24] = (uint8_t)(reason & 0xFF);
    // flags (u16)
    pkt[32] = (uint8_t)(flags & 0xFF);
    // ifname "wl0" at pkt[48]
    memcpy(pkt + 48, "wl0", 3);
    // ifidx = 0, bsscfgidx = 0
    // Send 64 bytes (fits in one bulk packet)
    if (tud_vendor_write(pkt, 64) == 64) {
        tud_vendor_flush();
    }
}
...
static uint16_t prepare_bcdc_response(uint8_t *buf, uint16_t max_len) {
    ...
    if (effective_cmd < 256) {
        switch (effective_cmd) {
            case BRCMF_C_UP:
                ...
                send_bulk_event(54, 0, 0, 1);    // WLC_E_IF, status=SUCCESS, flags=UP
                break;
            case BRCMF_C_DOWN:
    ...
    if (effective_cmd == BRCMF_C_GET_VAR) {
        ...
        // ===== WiFi association state (required for "ready but not connected") =====
            ...
            payload_len = copy_u32_payload(payload, max_payload, 0);  // not associated
