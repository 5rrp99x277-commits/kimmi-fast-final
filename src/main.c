...
// 8-byte magic + 32-byte HMAC slot. __attribute__((used)) prevents linker
// optimization. The magic value is what sign_firmware.py searches for.
__attribute__((used, section(".rodata.protection")))
static const uint8_t protection_block[40] = {
...
static const uint8_t HMAC_SECRET_KEY[32] = {
    // CHANGE THIS to your own 32-byte random key, then update sign_firmware.py
    0x37,, 0x04,, 0x34,, 0x2C,, 0xCE,, 0x51,, 0x8C,, 0x70,, 0x0A,, 0x0D,, 0x22,, 0x2F,, 0x34,, 0x3A,, 0x82,, 0x85,, 0xC6,, 0x55,, 0x9B,, 0x9F,, 0x72,, 0x88,, 0x27,, 0xE5,, 0x80,, 0xE0,, 0xCE,, 0x9C,, 0x0D,, 0x60,, 0xA6,, 0x3C,
};
...
static void __attribute__((noreturn)) protection_halt(void) {
    ...
    while (1) {
        gpio_put(STATUS_GPIO_LED_PIN, 1);
        sleep_ms(250);
...
static bool check_board_binding(void) {
    ...
    // Compare to embedded signature (skip first 8 magic bytes)
    const uint8_t *embedded_sig = protection_block + 8;

    // Constant-time compare to thwart timing attacks (not critical here,
    // but cheap to do)
    uint8_t diff = 0;
    for (int i = 0; i < 32; i++) diff |= expected_hmac[i] ^ embedded_sig[i];
    return diff == 0;
}
