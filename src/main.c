...
static bool ep3_in_opened = false;  // Bluetooth dummy interrupt endpoint
...
//--------------------------------------------------------------------
// Open EP3 IN (Bluetooth dummy interrupt) endpoint
// This is a dummy endpoint so Canon sees a BT interface like real CYW4373.
// We never send data on it — just keep it open.
//--------------------------------------------------------------------
static void open_ep3_in(void) {
    if (ep3_in_opened) return;
    tusb_desc_endpoint_t ep3_desc;
    ep3_desc.bLength          = sizeof(tusb_desc_endpoint_t);
    ep3_desc.bDescriptorType  = TUSB_DESC_ENDPOINT;
    ep3_desc.bEndpointAddress = 0x83;
    ep3_desc.bmAttributes.xfer  = TUSB_XFER_INTERRUPT;
    ep3_desc.bmAttributes.sync  = 0;
    ep3_desc.bmAttributes.usage = 0;
    ep3_desc.wMaxPacketSize   = 16;
    ep3_desc.bInterval        = 9;
    bool ok = usbd_edpt_open(0, &ep3_desc);
    if (ok) {
        ep3_in_opened = true;
        printf("EP83 (BT dummy) open: OK\r\n");
        return;
    }
    if (usbd_edpt_busy(0, 0x83)) {
        ep3_in_opened = true;
        printf("EP83 (BT dummy) open: OK (already active)\r\n");
    } else {
        printf("EP83 (BT dummy) open: FAIL\r\n");
    }
}
