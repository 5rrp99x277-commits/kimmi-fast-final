...
#define BT_TRIGGER_PIN  3   // GPIO3: HIGH for 3s → second RP2040 (BT) disconnects from USB
...
static bool last_cmd_was_down = false;  // true if DOWN came before UP
static uint32_t bt_show_deadline_ms = 0; // deadline to show BT if no UP (WiFi OFF)
static uint32_t bt_pulse_end_ms = 0;    // non-blocking pulse: time to lower GPIO
...
static void send_bulk_event(uint32_t event_type, uint32_t status, uint32_t reason, uint32_t flags) {
    ...
    printf("EP82 event %lu sent: %u/%u bytes (BCDC+eth+bcmeth+msg+pay)\r\n",
...
static void send_empty_escan_result(void) {
    ...
    printf("Empty escan result sent (event=69, status=0, 0 networks)\r\n");
}
...
static void send_scan_complete_event(void) {
    ...
    printf("Scan complete event sent (event=52, status=SUCCESS=0)\r\n");
}
...
static uint16_t prepare_bcdc_response(uint8_t *buf, uint16_t max_len) {
            ...
            case BRCMF_C_UP:
                {
                    ...
                    // Read ioctl parameter from BCDC payload
                    uint32_t up_param = 0;
                    if (payload_len >= 12) {
                        up_param = buf[8] | (buf[9] << 8) | (buf[10] << 16) | (buf[11] << 24);
                    }
                    printf("\r\n===== BRCMF_C_UP param=%lu =====\r\n", up_param);
                    printf("  up_time_ms    = %lu\r\n", up_time);
                    printf("  dongle_up     = %d -> 1\r\n", dongle_up);
                    printf("  boot_mode     = %d\r\n", boot_mode);
                    printf("  ep81_opened   = %d\r\n", ep1_in_opened);
                    printf("======================\r\n");

                    // BT hide pulse ONLY on param=1 (WiFi enabled by user).
                    // param=0 = firmware init (cold boot), don't touch BT.
                    // 20ms pulse: >9ms debounce, <50ms SHORT threshold.
                    if (up_param == 1 && (dongle_up || last_cmd_was_down)) {
                        gpio_put(BT_TRIGGER_PIN, 1);
                        bt_pulse_end_ms = up_time + 20;
                        printf("BT hide pulse 20ms (user enable)\r\n");
                    }
                    last_cmd_was_down = false;

                    dongle_up = true;
                    bt_show_deadline_ms = 0;  // Cancel BT show timer — UP arrived
                    last_heartbeat_ms = up_time;  // reset heartbeat timer
                    send_interrupt_event();          // short ping on EP81

                    // Defer BRCMF_E_IF event by ~30ms — must arrive BEFORE
                    // host sends BRCMF_C_DOWN (race condition observed in log).
                    // Real firmware sends async IF quickly; 30ms is enough for
                    // TinyUSB to flush EP82 bulk IN before next control xfer.
                    pending_if_event_ms = up_time + 30;
                }
                break;
            case BRCMF_C_DOWN:
                {
                    uint32_t down_time = to_ms_since_boot(get_absolute_time());
                    printf("RUNTIME BRCMF_C_DOWN -> OK (cancelling pending IF event)\r\n");
                    dongle_up = false;
                    bt_show_deadline_ms = 0;  // Cancel timer
                    last_cmd_was_down = true;  // Mark: next UP is re-enable
                    pending_if_event_ms = 0;
                    // Long pulse 100ms = BT connect (show BT)
                    gpio_put(BT_TRIGGER_PIN, 1);
                    bt_pulse_end_ms = down_time + 100;
                    printf("[DOWN] BT show pulse 100ms\r\n");
                }
                break;
            case BRCMF_C_SCAN:
                printf("RUNTIME BRCMF_C_SCAN (50) -> OK, scheduling deferred scan complete\r\n");
                {
                    uint32_t now_ms = to_ms_since_boot(get_absolute_time());
            ...
            case BRCMF_C_SCAN_RESULTS:
                {
                    ...
                    printf("RUNTIME BRCMF_C_SCAN_RESULTS (51) -> 0 results\r\n");
                }
                break;
            case BRCMF_C_DISASSOC:
                printf("RUNTIME BRCMF_C_DISASSOC (52) -> OK (not associated)\r\n");
                payload_len = 0;
                break;
    ...
    if (effective_cmd == BRCMF_C_GET_VAR) {
        ...
        // ===== Bluetooth/BLE (CYW4373L is WiFi-only — all disabled) =====
            ...
            printf("RUNTIME answer %s -> 0 networks (not associated)\r\n", iovar);
        ...
        if (strncmp(set_name, "p2p_", 4) == 0) {
            printf("RUNTIME P2P SET_VAR '%s' -> OK (P2P stays disabled)\r\n", set_name);
        } else {
        ...
        if (last_iovar_valid && (strncmp(set_name, "escan", 20) == 0 ||
            ...
            uint32_t now_ms = to_ms_since_boot(get_absolute_time());
            pending_scan_complete_ms = now_ms + 1500;
            printf("SCAN requested -> deferred scan complete in ~1500ms\r\n");
        }
...
void tud_suspend_cb(bool remote_wakeup_en) {
    ...
    printf("  boot_mode     = %d\r\n", boot_mode);
    printf("  dongle_up     = %d\r\n", dongle_up);
    printf("  pending_if    = %lu\r\n", pending_if_event_ms);
    printf("  pending_scan  = %lu\r\n", pending_scan_complete_ms);
    printf("  ep81_opened   = %d\r\n", ep1_in_opened);
    printf("=============================\r\n\r\n");
    status_led_off();
}
void tud_resume_cb(void) {
    ...
    // Short pulse: BT hide only if WiFi was active before suspend.
    // Cold boot WiFi OFF → dongle_up=false → no pulse (BT already hidden).
    if (dongle_up) {
        gpio_put(BT_TRIGGER_PIN, 1);
        bt_pulse_end_ms = now + 20;
        printf("[RESUME] BT hide pulse 20ms\r\n");
    }

    // If scan was pending during suspend, send events immediately after resume
    if (pending_scan_complete_ms != 0) {
        pending_scan_complete_ms = now + 50;  // 50ms after resume
        printf("[RESUME] Scan pending — rescheduled to %lu (+50ms)\r\n", pending_scan_complete_ms);
    }
    if (!boot_mode) {
        status_led_runtime();
        send_interrupt_event();
...
static void switch_to_runtime_reconnect(void) {
    ...
    // Restart BT show timer after runtime reconnect.
    // If WiFi is OFF, no BRCMF_C_UP will arrive and timer will show BT.
    bt_show_deadline_ms = to_ms_since_boot(get_absolute_time()) + 5000;
    printf("BT show timer restarted: %lu (+5s if no UP)\r\n", bt_show_deadline_ms);
...
int main(void) {
    ...
    // BT trigger: default LOW. HIGH for 3s during BRCMF_C_UP tells
    // the second RP2040 (BT dummy) to disconnect from USB.
    gpio_init(BT_TRIGGER_PIN);
    gpio_set_dir(BT_TRIGGER_PIN, GPIO_OUT);
    gpio_put(BT_TRIGGER_PIN, 0);  // LOW = BT stays connected
    ...
    // Start 5-second timer: if no BRCMF_C_UP arrives, WiFi is OFF → show BT
    bt_show_deadline_ms = to_ms_since_boot(get_absolute_time()) + 5000;
    printf("BT show timer: %lu (+5s if no UP)\r\n", bt_show_deadline_ms);

    while (1) {
        tud_task();
        drain_vendor_out();

        // BT show timer: if no UP after 5s, WiFi is OFF → show BT
        if (bt_show_deadline_ms != 0) {
            uint32_t now = to_ms_since_boot(get_absolute_time());
            if (now >= bt_show_deadline_ms) {
                bt_show_deadline_ms = 0;
                // Long pulse 100ms = BT show (WiFi is OFF, BT should be visible)
                gpio_put(BT_TRIGGER_PIN, 1);
                bt_pulse_end_ms = now + 100;
                printf("[TIMER] BT show pulse (WiFi OFF in settings)\r\n");
            }
        }

        // Non-blocking pulse drop: lower GPIO when pulse timer expires
        if (bt_pulse_end_ms != 0) {
            uint32_t now = to_ms_since_boot(get_absolute_time());
            if ((int32_t)(now - bt_pulse_end_ms) >= 0) {
                gpio_put(BT_TRIGGER_PIN, 0);
                bt_pulse_end_ms = 0;
            }
        }
        ...
        if (suspend_time_ms != 0 && !boot_mode) {
            ...
            if (since_suspend == 3000) {
                printf("[WARN] 3s since USB SUSPEND, dongle_up=%d\r\n", dongle_up);
            } else if (since_suspend == 4500) {
                printf("[WARN] 4.5s since USB SUSPEND — timeout approaching! dongle_up=%d\r\n", dongle_up);
            } else if (since_suspend == 6000) {
                printf("[WARN] 6s since USB SUSPEND — timeout exceeded! dongle_up=%d\r\n", dongle_up);
            }
        }
        ...
        if (pending_if_event_ms != 0 && !usb_suspended) {
            ...
            if (time_left <= 0) {
                ...
                printf("  actual_delay  = %d ms (planned 150ms)\r\n", -time_left + 150);
                printf("  dongle_up     = %d\r\n", dongle_up);
                printf("  boot_mode     = %d\r\n", boot_mode);
                printf("  Sending WLC_E_IF (54) via EP82...\r\n");
                send_brcmf_if_event();
                printf("=====================================\r\n\r\n");
            }
        }
        ...
        if (pending_scan_complete_ms != 0 && !usb_suspended) {
            ...
            if (time_left <= 0) {
                ...
                printf("  Sending empty escan result + scan complete...\r\n");
